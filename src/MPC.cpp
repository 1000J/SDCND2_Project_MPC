...
//      fg[0] += CppAD::pow(vars[v_start + i] - ref_v, 2);
    ...
    // minimize the value gap betw seq actuations
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // set upper/lower limits of non-actuators to the max negative and positive
  for (unsigned int i=0; i<delta_start; i++) {
    vars_lowerbound[i] = -1.0e19;
    vars_upperbound[i] =  1.0e19;
  }

  // set upper/lower limits of delta to -25 to 25 degrees
  ...
  std::cout << "Before solution push back" << std::endl;
  ...
  std::cout<<"After solution push back"<<std::endl;
